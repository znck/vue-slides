import { AnimationEngine } from '@components'
  ...
  prepareSlides({ state, commit }, vnodes) {
    ...
    return vnodes.map((vnode, index) => {
      ...
      if (oldSlide) {
        ...
        oldSlide.meta.component.props.vnode.default = () => vnode

        return oldSlide
      }

      console.log(index, { ...vnode }) // eslint-disable-line

      const slide = {
        steps: 0,
        meta: Object.freeze({
          vnode,
          component: createComponent(vnode)
        })
      }
...
function createComponent(vnode) {
  ...
  if (vnode.componentOptions) {
    // console.log(vnode.componentOptions.propsData) // eslint-disable-line
  } else {
  ...
  return {
    ...
    render(h, { data, props }) {
      return h(AnimationEngine, { propsData, ...data }, [props.vnode])
    }
  }
}
