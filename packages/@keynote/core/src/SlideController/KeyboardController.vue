<template>
  <VirtualElement
    ...
    @keyup.PageUp="dispatch('step.prev')"
    @keyup.left="dispatch('step.prev')"
    @keyup.PageDown="dispatch('step.next')"
  ...
  >
    ...
    <template slot="controls">
      <slot name="controls"/>
      <div class="goto" v-if="indexKey > -1" key="keyboard-goto">{{ indexKey }}</div>
      <div class="status" v-if="!isActive">
        <span class="icon-no-keyboard" aria-label="No keyboard controls">
          <svg class="keyboard" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512">
            <path
          ...
          <svg class="ban" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
            <path
  ...
  mounted() {
    ...
    const handler = event => {
      if (event.code === 'PageDown') {
        this.dispatch('step.next')
      } else if (event.code === 'PageUp') {
        this.dispatch('step.prev')
      }
    }
